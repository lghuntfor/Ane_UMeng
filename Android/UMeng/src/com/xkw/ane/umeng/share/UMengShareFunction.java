...
public class UMengShareFunction implements FREFunction {

	/**
	 * 接收一个参数(需要调用方法的方法名),
	 * 使用反射通过传入的方法名执行UMeng对象对应的方法,
	 * 获取所需要的返回值信息
	 */
	@Override
	public FREObject call(FREContext pContext, FREObject[] params) {
		UMengShare umeng = new UMengShare(pContext);
		...
		try {
			...
			if(params.length>1){
				...
				Method method = UMengShare.class.getMethod(methodName, classes);
				value = method.invoke(umeng,args).toString();
			} else {
				Method method = UMengShare.class.getMethod(methodName);
				value = method.invoke(umeng).toString();
			}
