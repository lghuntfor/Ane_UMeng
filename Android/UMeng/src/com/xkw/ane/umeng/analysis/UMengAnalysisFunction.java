...
public class UMengAnalysisFunction implements FREFunction {
	...
	public FREObject call(FREContext pContext, FREObject[] params) {
		UMengAnalysis umeng = new UMengAnalysis(pContext);
		...
		try {
			...
			if(params.length>1){
				...
				Method method = UMengAnalysis.class.getMethod(methodName, classes);
				value = method.invoke(umeng,args).toString();
			} else {
				Method method = UMengAnalysis.class.getMethod(methodName);
